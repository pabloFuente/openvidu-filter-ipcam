...
var role = 'MODERATOR'; 	// ['SUBSCRIBER', 'PUBLISHER', 'MODERATOR']
...
function joinSession() {
	...
	getToken(mySessionId, role).then(token => {
		...
		session.connect(token, myUserName)
			.then(() => {
...
function appendUserData(videoElement, streamManager) {
	var userData = streamManager.stream.connection.data;
	...
	videoElement.addEventListener('click', function () {
		...
		if (!streamManager.videos.map(v => v.video).includes(mainVideo)) {
			...
			$('#main-video').fadeOut("fast", () => {
				...
				var nickname = streamManager.stream.connection.data;
...
function publishIpCamera() {
	var jsonBody = {
		...
		rtspUri: "rtsp://wowzaec2demo.streamlock.net/vod/mp4:BigBuckBunny_115k.mov"
	};
	return new Promise((resolve, reject) => {
		$.ajax({
			type: "POST",
			url: OPENVIDU_SERVER_URL + "/openvidu/api/sessions/" + session.sessionId + "/connection",
			data: JSON.stringify(jsonBody),
			headers: {
				"Authorization": "Basic " + btoa("OPENVIDUAPP:" + OPENVIDU_SERVER_SECRET),
				"Content-Type": "application/json"
